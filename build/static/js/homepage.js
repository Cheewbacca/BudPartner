...
var source = $(".our-products__images").data("images-source");
...
if (source) {
  $.ajax({
    ...
    $.each(
      $('.our-products__content ul li a:not(".subm")'),
      function (linkIndex) {
        $(this).mouseover(function () {
          if (linkIndex) {
            ...
            $.each(
              $(".our-products__image-box").children(),
              function (imgIndex) {
                $(this).fadeOut(300, () => {
        ...
        $(this).mouseleave(function () {
          if (linkIndex) {
            ...
            $(".our-products__content ul li:first-child a").addClass(
              "active-link"
            );
          }
        });
...
function portfolioAnimation() {
  if ($(window).scrollTop() > $(".portfolio__wrapper").offset().top) {
    ...
    portfolio__galery.css({
      transform:
        "translate(0px, " + (scrolled * 0.1 - window.innerHeight / 2) + "px)",
    });
  }
}
...
if ($(window).width() <= 768) {
  $(".top-section .active-link").appendTo(
    $(".top-section .active-link").parent().siblings(".choose-us__content")
  );
}
